...
public class OrderSummaryServiceImpl implements OrderSummaryService {
    ...
    public DailyOrderSummaryResponse getDailySummary(LocalDate date, Long departmentId) {
        ...
        List<OrderSummaryItemResponse> items = rawData.stream().map(row ->
                OrderSummaryItemResponse.builder()
                        .userId((Long) row[0])
    ...
    public byte[] exportDailyExcel(LocalDate date, Long departmentId) {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook();
            ...
            Sheet sheet = workbook.createSheet("Tổng hợp suất ăn " + date.format(DateTimeFormatter.ofPattern("dd-MM-yyyy")));
            ...
            for (int i = 0; i < headers.length; i++) {
                ...
                if (i == 2) minWidth = 25 * 256; // Họ tên
                if (i == 3) minWidth = 25 * 256; // Phòng ban
    ...
    public byte[] exportMonthlyMatrixExcel(int month, int year, Long departmentId) {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook();
            ...
            for (int i = 0; i < summary.getItems().size(); i++) {
                ...
                cellRemaining.setCellValue(item.getRemainingAmount() != null ? item.getRemainingAmount().doubleValue() : 0.0);
            ...
            totalAmountCell.setCellValue(summary.getTotalAmount() != null ? summary.getTotalAmount().doubleValue() : 0.0);
            ...
            totalRemainingCell.setCellValue(summary.getTotalRemaining() != null ? summary.getTotalRemaining().doubleValue() : 0.0);
            ...
            for (int col = 0; col < nextColIdx + 5; col++) {
                ...
                if (col == 1) minWidth = 25 * 256; // Name
                if (col == 2) minWidth = 20 * 256; // Department
                if (currentWidth < minWidth) {
            ...
            String[] pmcHeaders = {"Ngày", "Suất thường", "Suất tăng cường", "TỔNG", "Số tiền cần TT cho PMC", "Ghi chú"};
            ...
            BigDecimal pmcNormalPrice = systemConfigRepository.findByConfigKey("PMC_NORMAL_MEAL_PRICE")
                    ...
                    .orElse(BigDecimal.valueOf(25000));
